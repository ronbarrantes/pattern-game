...
uint8_t choose_level(
    ...
    // Keep all LEDs on while waiting for a level selection
    for (uint8_t i = 0; i < pb_length; i++) {
        set_led(pb_arr[i], true);
    }
...
int main(void) {
    ...
    game_length =
        choose_level(pb_arr, pb_length, &seed);
